...
from splines import obtener_splines,evaluar_splines_c,normlzr_dist, cinematicaInversa
...
xy = np.load('corazon.npy') # Cargar trayectoria

# Elegir pose inicial

q = cinematicaInversa(xy[0][0], xy[1][0]) # Punto inicial de la trayectoria

# q = cinematicaInversa(0, -20) # Punto arbitrario
...
axs[0].plot(t_plot[0,:c], qp_plot[0,:c]*4, 'r-', linewidth=1.5, label="M1")
axs[0].set_ylabel("rad/s")
axs[0].set_title("Mot Vel")
axs[0].grid()
axs[0].legend()

axs[1].plot(t_plot[0,:c], qp_plot[1,:c]*4, 'r-', linewidth=1.5, label="M2")
axs[1].set_ylabel("rad/s")
axs[1].set_xlabel("tiempo [s]")
axs[1].grid()
axs[1].legend()
